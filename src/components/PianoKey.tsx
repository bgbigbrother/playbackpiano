...
const WhiteKeyButton = styled(Button, {
  ...
  '&:hover': {
    backgroundColor: isPressed ? '#d0d0d0' : '#f5f5f5',
    boxShadow: isPressed 
      ? 'inset 0 2px 4px rgba(0,0,0,0.2)' 
...
const BlackKeyButton = styled(Button, {
  ...
  transition: 'all 0.1s ease-in-out',
  '&:hover': {
    backgroundColor: isPressed ? '#444444' : '#222222',
    boxShadow: isPressed 
      ? 'inset 0 2px 4px rgba(255,255,255,0.1)' 
  ...
  if (isBlack) {
    return (
      <BlackKeyButton
        ...
        aria-label={`Piano key ${note}${keyboardKey ? `, keyboard key ${keyboardKey}` : ''}`}
      >
...
// Only re-render when note, isPressed, isBlack, or width changes
